<?php
	...
	if(isset($_POST['btnSnd'])) {
		if("Send"==$_POST['btnSnd']) {
			...
			$b1 = "SELECT empnum FROM tbltray WHERE empnum='$emp'";
			$b2 = mysql_query($b1);
			...
			if($c1>=1 || $c2>=1) {
				die("
					...
					window.location='add_trainor.php';
					</script>");
			}
			else {
				if((($_FILES["txtFle"]["type"] == "image/gif") ||
					...
					if($_FILES["txtFle"]["error"] > 0) {
						...
						if(file_exists("portrait/". $_FILES["txtFle"]["name"])) {
							...
							/* Get the highest id value and copy to tbltray */
							$e1 = "SELECT id FROM tbluser ORDER BY id DESC LIMIT 1";
							$e2 = mysql_query($e1);
							$e3 = mysql_fetch_assoc($e2);
							$e4 = $e3['id'];
							
							/* Insert data to tbltray */
							$f1 = "INSERT INTO tbltray(address,depart,empnum,gender,bday,cellnum,status,dhired,pichref,section,ctrlnum)
								   VALUES('$adr','$dep','$emp','$gdr','$dt1','$cel','$sta','$dt2','$fle','$sec','$e4')";
							mysql_query($f1);
							...
							echo("Save in: ". "portrait/" . $_FILES["txtFle"]["name"]);
