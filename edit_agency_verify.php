	...
	if(isset($_POST['btnUpd'])) {
		if("Update"==$_POST['btnUpd']) {
			...
			if($a3>=1) {
				...
				window.location='edit_agency.php?id1=$sid&id2=$grp';
				</script>");
			}
			else if($fle!=''){
				if((($_FILES["txtFle"]["type"] == "image/jpg" ) ||
					...
					if($_FILES["txtFle"]["error"] > 0 ) {
						...
						if(file_exists("portrait/".$_FILES["txtFle"]["name"])) {
							...
							$b2 = "UPDATE tblagency SET address='$adr',depart='$dep',gender='$gdr',bday='$bdy',cellnum='$cel',
									status='$sta',dhired='$dhr',pichref='$fle',section='$sec',eoc='$eoc' WHERE ctrlnum='$sid'";
							mysql_query($b2);
				...
				$b2 = "UPDATE tblagency SET address='$adr',depart='$dep',gender='$gdr',bday='$bdy',cellnum='$cel',
				status='$sta',dhired='$dhr',section='$sec',eoc='$eoc' WHERE ctrlnum='$sid'";
				mysql_query($b2);
